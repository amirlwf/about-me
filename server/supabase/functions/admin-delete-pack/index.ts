...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Get pack info for response message
    ...
    // Delete all configs in the pack first (FK won't cascade because ON DELETE SET NULL)
    const { error: deleteConfigsError } = await supabaseAdmin
      .from("vless_configs")
      .delete()
      .eq("pack_id", pack_id)

    if (deleteConfigsError) {
      return new Response(JSON.stringify({ error: "خطا در حذف کانفیگ‌های بسته" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } })
    }

    // Delete the pack itself
    const { error: deletePackError } = await supabaseAdmin
      .from("config_packs")
      .delete()
      .eq("id", pack_id)

    if (deletePackError) {
      return new Response(JSON.stringify({ error: "خطا در حذف بسته" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } })
    }

    return new Response(JSON.stringify({
      message: `✅ بسته "${packName}" و تمام کانفیگ‌های آن با موفقیت حذف شد`,
